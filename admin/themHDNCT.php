...
<?php include('../includes/body-left-admin.php');?>
    ...
    <div id="form">
    <h2>Thêm đơn hàng nhập chi tiết</h2>
    <br />
    <br />
                <?php 
					if($_SERVER['REQUEST_METHOD'] == 'POST'){//neu gia tri ton tai thi xu ly form
						...
						if(empty($errors)){
							...
							hoadonnhap(ID_DH,ID_MatHang,ID_NSX,SoLuong,GiaNhap,Thue) 
							values('$madh','$mamh','$mansx','$soluong',$gia,'$thue')";
                ...
                <form id="add_cat" action="" method="post">
                	<fieldset>
                    	<legend>Thêm đơn hàng nhập chi tiết</legend>
                        	<div align="center">
                                    ...
                                    $r_makh = mysql_query($q_makh);								
                                        if(mysql_num_rows($r_makh) != 0){
                                            while($num = mysql_fetch_array($r_makh)){
                                                if(isset($maldh) && $maldh == $num['ID_DH']) 
